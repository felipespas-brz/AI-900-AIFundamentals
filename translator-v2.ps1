...
$result = Invoke-Webrequest -Method Post `
          -Uri "$endpoint/translate?api-version=3.0&from=en&to=fr&to=it&to=zh-Hans&to=pt-BR" `
          -Headers $headers `
          -Body $body 
...
$brazilian = $analysis.translations[3] 
Write-Host ("Original Text: $text`nFrench Translation: $($french.text)`nItalian Translation: $($italian.text)`nChinese Translation: $($chinese.text)`nBrazilian: $($brazilian.text)`n")
...
$result = Invoke-Webrequest -Method Post `
          -Uri "$endpoint/translate?api-version=3.0&from=en&to=pt-BR" `
          -Headers $headers `
          -Body $body 
# 
$analysis = $result.content | ConvertFrom-Json
# $french_translation = $analysis.translations.text
$brazilian_translation = $analysis.translations.text
Write-host ("Translated text: '$brazilian_translation'")
# 